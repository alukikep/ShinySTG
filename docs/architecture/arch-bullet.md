...
CollisionService 对普通敌人和 Boss 的玩家弹命中路径，在伤害回调前请求生成残影，
保证伤害回调清场或回收子弹之前完成外观快照。原子弹继续使用延迟回收，
残影不参与碰撞，也不运行 Modifier。残影由场景特效池独立管理，
复制单个 SpriteRenderer 的外观与材质属性；多层视觉和激光不在通用残影范围内。
配置、材质透明度要求及附带音效见 [战斗特效](../../Assets/Scripts/Effects/README.md)。
