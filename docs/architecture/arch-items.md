...
吸附启动范围由 PlayerHitbox 根据玩家高低速模式选取；切换到较小范围不取消已有追踪。
ItemDropService 可配置世界 Y 收点线。玩家达到或超过收点线时进入持续收点状态，现有及后续生成的道具均可开始吸附；
玩家回到收点线下方后，尚未开始吸附的道具不再因上线收点启动，已经开始吸附的道具继续追踪。
收点状态只影响吸附启动，不复制奖励结算；所有奖励仍由 ItemPickup.TryCollect 统一处理。
ItemDefinition 可为每种道具配置独立的 PickupSfx；TryCollect 在奖励成功锁定后按拾取位置播放一次，
音效不跟随对象池实例，空配置或音频服务缺失不影响奖励。密集拾取的合并和限流由 SfxCue 自身规则负责。
模式判断与旧配置兼容规则见 [玩家系统](./arch-player.md#道具与玩家资源)。
...
普通敌人正常击杀和 KillEnemiesCommand 强制死亡共用单次死亡掉落路径，按各自 DropProfile 结算。
行为自毁、出界自毁和 ClearEnemiesCommand 清场均不提交掉落；清除时播放死亡特效不会改变奖励语义。
普通敌人的清除使用销毁而非道具对象池，生命周期见 [敌人 AI](./arch-enemy-ai.md#自毁与出界)。Boss 奖励按实际阶段退出结算，
