...
死亡演出开始时，协调组件终止当前 Bomb，释放动作执行器、移除 Bomb 自己的无敌锁并清空
临时追踪状态，不执行正常结束动作，也不回收已生成的子弹。死亡清弹仍由死亡指令配置负责。
成功复活后，协调组件将库存覆盖为配置数量；普通复活与续关共用此规则，最后一命死亡时
不重置库存。库存更新在 CompleteRevive 返回后进行，因此 OnRevive 回调仍读取重置前的库存。
资源组件仅在库存实际变化时通知 HUD；成功决死未进入死亡流程，不触发上述重置。
