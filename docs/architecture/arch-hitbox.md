...
玩家参与判定以 PlayerHealth.CanInteract 为准，不能只检查生命是否归零。
死亡演出期间即使仍有生命，也不进入玩家伤害网格、不受伤或擦弹；拾取与吸附同样排除。
子弹与激光遍历重新读取可交互及无敌状态，避免同帧首次命中后继续使用旧状态。
激光跳过玩家判定时仍执行自身出界回收，死亡流程不负责停止世界或销毁所有弹幕。
