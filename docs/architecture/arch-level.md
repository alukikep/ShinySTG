...
当前已完成序列启动、自动换关、末关结果页、内存结算和战斗限制／清场，尚未写入磁盘存档。失败不暂停游戏时间，死亡演出可以继续；Game Over 界面尚未接入。配置有效的 StageFadePrototype 时复用显式 BattleArea 裁剪与机体绘制，否则使用全屏渐变。

GameFlowController 在结束事件完成后等待结算、清场及必要的玩家复活，再推进序列。换关保留场景、玩家位置和资源；宿主另持 BattleRestriction，覆盖下一关 BeginLevel 释放旧令牌到揭幕完成的窗口。Bootstrap 的下一关准备入口只替换时间轴，沿用结算订阅。轮次检查拒绝过期的换关流程；异常进入可返回标题的错误状态。末关展示累计分数和逐关成绩，支持重开本局及返回标题。重开沿用本次固定的开局请求重新加载场景，重建玩家和 Session。
