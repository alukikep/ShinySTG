...
当前支持序列启动、逐关计分确认、关间转场、整局结果页、内存结算和战斗限制／清场，成绩尚未写入磁盘存档。正式流程已接入暂停与死亡续关，配置及操作见下方开局流程说明。配置有效的 StageFadePrototype 时复用显式 BattleArea 裁剪与机体绘制，否则使用全屏渐变。

GameFlowController 在结束事件完成后等待结算、清场及必要的玩家复活，再展示单关计分。StageResultView 只读取已记录的快照；确认、轮次校验和序列推进归 GameFlowController 管理。等待确认独立于加载及整局结果状态，不重复结算或启动换关。界面出现和失焦后需要松开并重新按 Z；普通关确认后进入转场，末关（含单关入口）确认后进入整局结果页。失败和中止不进入通关计分。

换关保留场景、玩家位置和资源；计分展示起持有 PlayerControlLock，普通关在下一关揭幕完成后释放，末关沿用到整局结果页。计分展示本身不修改游戏时间。宿主另持 BattleRestriction，覆盖下一关 BeginLevel 释放旧令牌到揭幕完成的窗口。Bootstrap 的下一关准备入口只替换时间轴，沿用结算订阅。Session 与 AttemptId 检查使调试重开或宿主失效后的旧界面和旧确认失效；异常进入可返回标题的错误状态。整局结果页展示累计分数和逐关成绩，支持重开本局及返回标题。重开沿用本次固定的开局请求重新加载场景，重建玩家和 Session。

### 战斗限制与清场

结束定案时 Controller 持有 `BattleRestriction` 令牌，屏蔽射击、伤害、擦弹、拾取与新增掉落，停止敌人行为及新弹幕生成；该限制本身仍允许存活玩家移动和低速，正式流程的计分与转场另由 PlayerControlLock 锁定操作。它不修改游戏时间、玩家资源或对话控制锁。Bomb 当前只有库存，未来释放入口必须检查同一限制。
...
重开先清场再释放 Controller 自己的令牌；销毁 Controller 同样释放，其他宿主的令牌不受影响。清场本身幂等且临时持有限制，取消回调不能生成新的弹幕或掉落。调用方必须在碰撞遍历之外使用清场／重开入口。正式换关在揭幕完成前继续持有宿主的战斗限制。
