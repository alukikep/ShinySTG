...
| 新发射扩展(BaseAngle / 瞄准玩家 / 瞄准 Boss / 每发旋转 / 振荡 ...) | `Assets/Scripts/Bullet/FireExtension/` | 子类继承 `FireExtension` + 加 `[SRName("FireExtension/<名字>")]`,自动出现在所有 FirePattern 资产的下拉菜单。**Pipeline 模型**:每个模块实现 `ProcessAngle(from, baseRotationRad, currentAngleRad)`,数组按顺序串成"角度管道"(详见 ARCHITECTURE §3.1)。**位置偏移**:只在 Base 模块上,通过 `PositionOffset: Vector2` 字段在世界坐标系下设置炮口偏移(由 Resolver 在 pipeline 入口处应用)。**累加型扩展**(需要"每次开火后累加偏移"):除了 override `ProcessAngle` 外,还要 override 基类的 `public virtual void OnFireGroupTriggered(int fireCount)` 钩子更新自己的累加状态—— `BulletPool.FireGroup` 入口会按 `(pattern 资产 ref + FireExtensions 数组元素 ref)` 维护字典,对每个非 null 元素自动调用该钩子;`fireCount` 从 1 起累加。**已内置** `AccumulatingOffsetAngleFireExtension`(本批次 BaseOffset SR 多态 + StepOffset 累加 + 每发 OffsetPerBullet 三维度叠加,详见 fire-pattern §3.1.1 + `Assets/Scripts/Bullet/FireExtension/AccumulatingOffsetAngleFireExtension.cs`)。**与 Extra/Angle Offset 的区别**:本扩展影响 FirePattern 自身每次开火方向,Extra/Angle Offset 只影响母弹分裂那一刻传给分裂弹的 rotationRad;两者独立可叠加 |
| 新敌人行为(动画 / 隐身 / 加血) | `Assets/Scripts/Enemy/AI/Actions/` | 子类继承 `EnemyAction`,加 `[SRName("Action/<名字>")]` |
| **新 Action Duration 策略**(区间随机 / 曲线采样 / 按难度动态缩放)
| `Assets/Scripts/Enemy/AI/ActionDurationConfig.cs` | 子类继承 `ActionDurationConfig` + 加 `[Serializable, SRName("Duration/<名字>")]` + override `Sample()`(返回本次要施加的 Duration 秒,必须 >= 0)。**已内置** `FixedActionDuration`(精确值,默认,与旧 `float Duration` 等价)+ `RandomRangeActionDuration`(区间随机,Min/Max 秒,每次进入行为抽一次,期间固定)。由 `BehaviorFlowRuntime.AdvanceTo` 在切到 Action 时自动调 Sample,结果缓存到 `EnemyAction.CurrentDuration`。**老 .asset 兼容**:旧 `Duration: X`(float)走 `[FormerlySerializedAs("Duration")]` 迁移到隐藏 `_legacyDuration`,SR 字段为 null 时 Runtime 兜底用 legacy 值,行为 100% 等价。典型用法:Boss 节奏混乱(`FireAction` = Random Range(2, 4))、玩家反应窗口抖动(`WaitAction` = Random Range(0.5, 1.5))、弹性巡逻(`MoveAction` = Random Range(3, 5))。详见 `ARCHITECTURE.md` §4.0.5 |
| **新敌人发信号**(对接 BulletModifier 信号触发) | `Assets/Scripts/Enemy/AI/Actions/` | 子类继承 `EnemyAction`,加 `[SRName("Action/<名字>")]`。**已内置** `EmitSignalAction`(SRName `Action/Emit Signal`):在 BehaviorFlow 时间轴某点调 `BulletSignalBus.Emit(SignalName, origin)`,把 AI 节奏对齐到子弹 modifier 激活时机。3 个 EmitMode(OnEnterOnly / EveryTick / OnInterval)。典型用法:Boss 蓄力 Action.OnEnter 触发 `"boss_charge_finished"` → 全场分裂 modifier 立即开火。详见 ARCHITECTURE §2.8 + `Assets/Scripts/Bullet/BulletSignalBus.cs` |
| 新移动方式(贝塞尔 / 圆形 / 追踪 / **区域内随机直线**) | `Assets/Scripts/Enemy/AI/MoveBehaviours/` | 子类继承 `MoveBehaviour`,加 `[SRName("Move/<名字>")]`。**RandomWalkInRegionMove** 是最复杂示例:含区域 AABB 裁剪(MaxDistanceInsideBox helper)、子状态机(Moving/Idle)、三档速度曲线(Constant/FastToSlow/SlowToFast,公式 cos(t·π/2) / sin(t·π/2))、per-instance `Random` 种子(避免同步),参考它就能写其他"区域型"移动(圆形/多边形只要改 helper) |
| **新子弹出生雾化方法**(波形 / 径向膨胀 / 颜色渐变 / 拖尾) | `Assets/Scripts/Bullet/FirePattern/SpawnFog/` | 子类继承 `SpawnFogConfig` + 加 `[SRName("Spawn Fog/<名字>")] + [Serializable]`,**override `ApplyVisual(Bullet b, float t01, Vector3 baseLocalScale)`** 与 `ClearVisual(Bullet b, Vector3 baseLocalScale)`。视觉走 shader 的写 MaterialPropertyBlock 调 `Bullet.ApplyFogMaterialParams(fogAmount, fogColor)`(不要直接访问 `_fogMpb` 私有字段,生命周期由 Bullet 统一管理)。缩放务必走 `baseLocalScale × fogScale` 乘法,不要覆盖 `transform.localScale`,避免破坏 prefab 美术基准。详见 ARCHITECTURE §3.3 |
| **新子弹逻辑效果(加速 / 转向 / 减速 / 分裂 / 追踪)** | `Assets/Scripts/Bullet/` | 子类继承 `BulletModifier`,加 `[SRName("Modifier/<名字>")] + [Serializable]`,**override `ModifyCore(Bullet, dt)`**(不要 override `Modify`,它是 `sealed`)。**时间窗口已由基类自动提供**(详见 ARCHITECTURE §2.6):`Delay` / `Duration` / `OneShot` 字段直接用,无需自己写计时器;一次性逻辑 override `OnWindowEnter(Bullet)` 写;**窗口退出清理 override `OnWindowExitCleanup(Bullet)`**(不要 override `OnWindowExit`,基类会自动清 `b.AngularSpeed` 让子弹切直线,详情见 §2.6 钩子表)。引用类型字段要 override `Clone()` 深拷 |
| **新 modifier 启动触发器**(决定「何时进入时间窗口」) | `Assets/Scripts/Bullet/ModifierStartTrigger.cs` | 在 BulletModifier.StartTrigger 字段上挂子类(SRName `Trigger/<名字>`,与 `BaseOffsetStrategy` 同套路)。override `OnAttach(Bullet)` / `OnDetach(Bullet)` / `ShouldActivate(Bullet, elapsed)` 三个方法,订阅型 trigger 在 OnAttach 调 `BulletSignalBus.Subscribe`、OnDetach 调 `Unsubscribe`。**已内置** `DelayStartTrigger`(默认)/ `OnSignalStartTrigger`(订阅 BulletSignalBus 收到即激活)/ `DelayOrSignalStartTrigger`(二选一)。**★ vX 起 Duration 是「窗口内持续时长」,与 trigger 何时激活完全解耦**:信号型 trigger 在信号到达瞬间激活,持续 Duration 秒(此前 Duration 被误用为「出生后总预算」,导致信号型 + Duration>0 时永远进不了窗口)。详见 ARCHITECTURE §2.6 / §2.8 + `Assets/Scripts/Bullet/ModifierStartTrigger.cs` |
| **新子弹分裂模式**(按 FirePattern 再发射) | `Assets/Scripts/Bullet/FirePatternBulletModifier.cs` 或新文件 | 子类继承 `FirePatternBulletModifier`(基类已封装 `BulletPool.FireGroup` + `Pattern` / `InheritOwnerTeam` / `ExtraModifiers` 字段 + Clone 深拷),加 `[SRName("Modifier/<名字>")] + [Serializable]`。只需 override `ComputeCenterAngle(Bullet)` 决定分裂弹中线方向(默认 `b.SteerAngle`),触发时机复用基类的 `Delay` / `Duration` / `OneShot`(OneShot 子类 override `OnWindowEnter` 调基类 `FireOnce`;持续型 override `ModifyCore` 按 `Interval` 循环调)。**已内置** `FireOnEnterBulletModifier` (OneShot 延迟触发) 和 `FireOnDurationBulletModifier` (持续型按周期触发) |
| **新母弹 → 分裂弹信息传递策略** | `Assets/Scripts/Bullet/FirePatternBulletModifier.cs` 或新文件 | 子类继承 `FirePatternBulletExtra`(已有 `GetRotationOffset` / `OnFireTriggered` / `Clone` 三个虚方法),加 `[SRName("Extra/<名字>")] + [Serializable]`。**已内置** `NoFirePatternBulletExtra`(显式不传递占位) + `AngleOffsetFirePatternBulletExtra`(本批次偏移 + 累加偏移)。**新增 BaseOffset 策略**则继承 `BaseOffsetStrategy` + 加 `[SRName("Base Offset/<名字>")]`,实现 `Sample()`(返回本次要施加的偏移值,度);**已内置** `FixedBaseOffsetStrategy`(精确值) + `RandomRangeBaseOffsetStrategy`(区间随机,本颗母弹第一次 FireOnce 时抽一次后保持)。
**Synchronized 模式扩展**:`Extra` 子类可加一个 `BatchSample` 枚举字段(Independent / Synchronized) + `OnBatchFire()` 方法 —— 由 `BulletPool.FireGroup` 入口遍历 extraModifiers 找挂有本 Extra 的 `FirePatternBulletModifier` 子类时调用一次。Synchronized 模式下把抽样结果写进自己的 per-instance 字段;通过 MemberwiseClone 字段浅拷贝传到本批每颗母弹 modifier,实现「本批共享」。Independent 模式直接 return,行为与历史 100% 等价。无需改 Bullet / FirePattern(详见 ARCHITECTURE §2.5 AngleOffset.BatchSample 段) |
