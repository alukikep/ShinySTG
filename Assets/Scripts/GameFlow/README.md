...
最后一关同样先显示单关计分，按 Z 后显示整局结果页，包括总分和各关得分。上下键选择，Z / Enter 确认，Esc 返回标题，也可用鼠标。重新开始本局会使用本次固定的角色和关卡顺序重新加载场景，创建新玩家和 Session；返回标题清空本局状态。结果尚未写入磁盘存档。
...
局部渐变的配置与手动预览见 [转场说明](StageFadePrototype.md)。单关入口同样先显示单关计分，按 Z 后进入整局结果页；两种界面期间玩家移动被锁定，背景保持结束状态。单关计分仅接受 Z 确认，Enter、Esc 和鼠标不推进关卡；失焦后重新等待松键。计分展示本身不暂停游戏时间，也不额外增加通关奖励。

关卡结束和结算的权威规则见 [关卡架构](../../../docs/architecture/arch-level.md#结束与结算)。独立测试场景若没有通过 GameplayBootstrap 建立本局 Session，只有 LevelController 的结束与清场，不会提供计分确认、换关或整局结果页。
...
- 配置两关，各自设置通关条目；确认首关清场后停留计分界面，长按 Z 不跳过，松开再按只进入一次第二关，位置与资源继承，第二关得分从新基线计算。
- 计分期间失焦再聚焦需要重新松键；调试重开当前关后旧计分界面关闭，旧确认不能推进关卡。
- 分别验证有效 BattleArea 和未配置局部渐变的场景；揭幕前不能射击，揭幕后时间轴继续，背景和音乐切换符合配置。
- 最后一关与单关入口均先显示单关计分，按 Z 后进入整局结果页，继续长按不会误触重开；重开回到首关并重置资源，返回标题后再次开局无旧成绩或控制锁。
- 通关同帧耗尽残机时先等待续关；确认复活后才允许待定通关结算。尚有残机时等待自动复活完成后继续。

EditMode 的 `RunSessionTests` 覆盖序列校验、连续两关结算与资源继承、重复结算及宿主战斗限制；`StageResultFlowTests` 覆盖普通关与末关的清场等待、确认前不推进、重开后拒绝旧确认，以及输入重置后的松键保护。实际渐变、失焦和结果页交互仍需在 Play Mode 验收。
