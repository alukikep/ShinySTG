...
选择开始游戏后，选项先闪烁，黑幕从右向左覆盖屏幕，全黑时切换至角色选择页，再继续向左退出。角色页支持上下键选择，首尾循环和长按重复；切换时更新箭头、角色名称、简介及立绘，按 Z / Enter 确认配置好的角色后进入首关或已配置的关卡序列。按 X / Esc 经反方向黑幕返回开始菜单。设置和退出仍只显示占位确认提示。
...
验收：默认选中开始游戏；首尾循环；长按重复；长按确认只触发一次；转场中连续按键不重复跳转；全黑时切换页面；X / Esc 返回后可继续操作；失焦/恢复和 FrontEnd 禁用/启用不会误触或残留黑幕；Time.timeScale 为 0 时仍能操作。检查 4:3、16:9 和转场中窗口缩放下黑幕完整覆盖、文字无裁切。配置首关或关卡序列后，还需验证角色确认、场景切换、HUD、刷怪与音乐。关卡结束原因和单关结算见项目根目录 `docs/architecture/arch-level.md`。
