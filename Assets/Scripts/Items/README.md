...
6. 在 `ItemDropService` 的“上线收点”分组配置 `Enable Auto Collect` 和 `Auto Collect Line Y`。
   玩家达到或超过收点线时，现有及后续生成的道具都会开始吸附；回到收点线下方后，尚未开始吸附的新道具不再自动吸附，
   已经开始吸附的道具会继续追踪并正常拾取。
...
在各个 ItemDefinition 资产的 **Pickup Sfx** 中引用非循环 SfxCue，可为小 P、大 P、分数、
1UP 和 Bomb 分别配置声音；共用同一个 Cue 则共用其限流规则。留空保持静默。
创建音效见[音频配置](../Audio/README.md#快速配置)。

仅成功拾取时请求一次播放，使用拾取位置，声音不跟随回收的道具。拾取失败、超时或清场回收不播放；
音频系统缺失不会影响奖励。密集收 P 时可调整 Cue 的 Cooldown、Max Voices 和 Overflow，
不同 Cue 独立限流。满 Power 仍属于成功拾取，会请求拾取音效。
PlayerHealth 的 Power Up Sfx 仍用于火力升档，升档时可能与拾取音效同时播放，请分别配置。
...
普通敌人自毁/离场不掉落；Clear Enemies 即使开启死亡特效也不掉落。
Kill Enemies 按各敌人的 Death Drops 配置结算一次，仍遵循条目概率；配置入口见
[清除普通敌人](../GameActions/README.md#清除普通敌人)。
Boss 仅结算实际退出的阶段，不为跳过阶段补发奖励。
...
普通吸附启动后持续追踪；玩家不可拾取时停止追踪。上线收点由 `ItemDropService` 根据收点线维护，收点状态只影响吸附启动，
不新增独立奖励结算循环。收点线下未开始吸附的道具遵循原有下落或普通吸附规则，已开始吸附的道具继续追踪。
所有奖励仍通过 `ItemPickup.TryCollect` 结算，吸附使用 MoveTowards 防止越过玩家。
...
- 将玩家移动到收点线以上，确认场上及新生成道具持续吸附；回到收点线以下后，确认新道具不再自动吸附，
  但已开始吸附的道具仍能继续追踪并被拾取。
